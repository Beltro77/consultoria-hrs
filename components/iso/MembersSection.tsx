...
import {
  type ProjectMember, type MemberLogEntry,
...
function MemberRow({ member, onEdit, onDeleted }: {
      ...
      <div className="flex items-start gap-3 px-4 py-3">
        ...
        <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
          ...
          <button onClick={handleExpand} className="text-[10px] text-stone-400 hover:text-stone-600">
            {expanded ? 'Ocultar seguimiento ▾' : 'Ver seguimiento ▸'}
          </button>
        </div>
      </div>
    ...
    <div className="mx-4 mb-4 bg-white border border-stone-200 rounded-xl overflow-hidden">
      <div className="flex items-center justify-between px-4 py-3 border-b border-stone-50">
        <p className="text-[11px] font-semibold text-stone-400 uppercase tracking-wide">
          ...
          {members.length > 0 && <span className="ml-1.5 bg-stone-100 text-stone-500 rounded-full px-1.5 py-0.5 text-[10px]">{members.length}</span>}
        </p>
      ...
      {members.length === 0 && (
        <p className="text-xs text-stone-300 text-center py-6 italic">
